...
# Filtrar películas dirigidas por Christopher Nolan y extraer solo título y calificación
peliculas_nolan = list(map(
    lambda p: {"Titulo": p["Titulo"], "CalificaciónIMDB": float(p["CalificaciónIMDB"])},
    filter(lambda p: p["Director"] == "Christopher Nolan", peliculas)
))

# Imprimir la salida formateada como una lista de Python
print("[")
